...
for imp in range(impNumber):
    ...
    for file in filesImp:
        ...
        if df == 1:
            ...
            hp_candidates = [{'n_neighbors': range(1,20), 'weights': ['uniform', 'distance'],'p':[1,2],'algorithm': ['auto','ball_tree', 'kd_tree', 'brute']}]
            # Instantiate the grid search model
            num_folds = 3
            seed = 7
            kfold = KFold(n_splits=num_folds, random_state=seed, shuffle=True)
            grid_search = GridSearchCV(estimator=base_model, param_grid=hp_candidates, scoring='neg_mean_squared_error',cv=kfold, n_jobs=-1, verbose=2)
            ...
            #base_accuracy = 59.39291859218764
