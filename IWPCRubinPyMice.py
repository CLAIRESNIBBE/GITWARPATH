...
def main():
    ...
    for file in filesImp:
      ...
      y_pred_stack = sc.inverse_transform(stack.predict(X_test))
      stack2 = StackingCVRegressor(regressors=(rf, modelsk_NN, modelsk_DTR), meta_regressor=rf,
                                   use_features_in_secondary=True)
      stack2.fit(X_train, y_train)
      y_pred_stack2 = sc.inverse_transform(stack2.predict(X_test))
      ...
      y_pred_stack3 = sc.inverse_transform(stack3.predict(X_test))
      ...
      y_pred_stack4 = sc.inverse_transform(stack4.predict(X_test))
      stack5 = StackingCVRegressor(regressors=(SV, modelsk_GBR, modelsk_NN), meta_regressor=SV,
                                   use_features_in_secondary=True)
      stack5.fit(X_train, y_train)
      y_pred_stack5 = sc.inverse_transform(stack5.predict(X_test))
      ...
      impResults.append({'Imp': df, 'model': 'Fixed', 'predactual': dfKey, 'MAE': 0, 'PW20': 0, 'R2': 0, 'MAPE': 0, 'MLAR': 0,
                 'MALAR': 0})
      dfmod.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\MicePredictions\IWPCImputed_" + str(df) + ".csv", ";")
      ...
      for k in range(len(impResults)):
        ...
        #if model == 'IWPC':
            #print(model, doseTrue, dosePred2)
      ...
      resultsdf.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\ResultsPython\IWPCResults_" + str(df) + ".csv", ";")
      boot = 0
      samples = []
      metrics = []
      smpResults = []
    ...
    if df == impNumber:
            ...
            print("using Python MICE ", "imputations ", impNumber, "samples", number_of_samples, file = fileoutput)
            for k in range(len(models)):
                fieldname = models[k]['model']
