...
def traineval(est: Estimator, xtrain, ytrain, xtest, ytest, squaring, df):
    ...
    if est.identifier != "LR":  # tuning is not required for LR
        ...
        if est.identifier == "RF":
            start = time.time()
            maxfeatures = ['sqrt', 0.231921389, 0.31545350158196, 0.3270273536923972, 0.51121008192637868,
                           0.9140916130787694]
            nestimators = [12, 14, 38, 40, 53, 77, 83, 322, 405, 414, 691, 1340, 1866]

            def RF_Objective(trial):
                _min_samples_leaf = trial.suggest_int('min_samples_leaf', 1, 18, step=2)
                _min_impurity_decrease = trial.suggest_float('min_impurity_decrease', 0.0, 0.01, step=0.01)
                # _max_features = trial.set_user_attr('max_features', maxfeatures)
                _max_depth = trial.suggest_int('max_depth', 2, 4, step=2)
                _n_estimators = trial.suggest_int('n_estimators', 10, 2000, step=100)
