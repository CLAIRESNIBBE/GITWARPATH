...
def main():
    ...
    dfnew = pd.read_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\dataset1.csv", ",")
    dfmod = dfnew
    ...
    try:
        ...
        for df in range(len(implist)):
            ...
            #print(dfcurrent)
            print(dfcurrent.info())
            dfmod = dfcurrent
            #dfmod.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\TheDataSet1.csv", ";")
            ...
            impResults.append(
                {'Imp': df, 'model': 'Gage', 'predactual': dfKey, 'MAE': 0, 'PW20': 0, 'R2': 0, 'MAPE': 0, 'MLAR': 0,
                 'MALAR': 0})
            dfmod["WarPATH_Pred"] =  factor_WarPath['Intercept'] + factor_WarPath['Age'] * dfmod['AgeYears'] + \
                                     factor_WarPath['Weight'] *  dfmod['Weight_kg'] + factor_WarPath['TargetINR'] * dfmod['INR_Three']
            ...
            data = np.random.randint(10, size=(1000, 5))
            ...
            if True:
              ...
              for m in range(len(smpResults)):
                  if smpResults[m]['model'] == 'WarPATH':
                      print( smpResults[m]['MAE'])
              ...
              for i in range(len(metric_columns)):
                ...
                #df_WARPATH[current_metric].to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarPATH_metrics" + ".csv", ";")
                std = std_deviation(df_WARPATH[current_metric])
                if current_metric == 'MAE':
                    df_WARPATH[current_metric].to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarPATH_metricsMAE" + ".csv", ";")
                    print('WarPATH', 'std deviation of MAEs is ', std)
                if current_metric == 'PW20':
                    df_WARPATH[current_metric].to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarPATH_metricsPW20" + ".csv", ";")
                    print('WarPATH', 'std deviation of PW20s is ', std)
                if current_metric == 'R2':
                    df_WARPATH[current_metric].to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarPATH_metricsR2" + ".csv", ";")
                    print('WarPATH', 'std deviation of R squared values is ', std)
                ...
                std = std_deviation(df_IWPC[current_metric])
                ...
                std = std_deviation(df_GAGE[current_metric])
                ...
                std = std_deviation(df_FIXED[current_metric])
        ...
        for k in range(len(std_Dev)):
            ...
            std_Dev_Summ[modelpos][metric] += Wfactor**2
        ...
        for k in range(len(models)):
                fieldname = models[k]['model']
                mae_value = models[k]['MAE'] / 1
                stdpos = find(std_Dev_Summ, 'model', fieldname)
                varpos = find(variance_Summ, 'model', fieldname)
                mae_var = variance_Summ[varpos]['MAE']*2
                mae_std_dev = std_Dev_Summ[stdpos]['MAE']/1
                mae_CI_minus = round(mae_value - np.sqrt(mae_std_dev + mae_var),4)
                mae_CI_plus = round(mae_value + np.sqrt(mae_std_dev + mae_var),4)
                pw20_value = models[k]['PW20'] / 1
                pw20_std_dev = std_Dev_Summ[stdpos]['PW20']/1
                pw20_var = variance_Summ[varpos]['MAE']*2
                pw20_CI_minus = round(pw20_value-np.sqrt(pw20_std_dev + pw20_var),4)
                pw20_CI_plus = round(pw20_value +np.sqrt(pw20_std_dev + pw20_var),4)
                R2_value = models[k]['R2'] / 1
                R2_var = variance_Summ[varpos]['R2']*2
                R2_std_dev = std_Dev_Summ[stdpos]['R2']/1
                R2_CI_minus = round(R2_value - np.sqrt(R2_std_dev + R2_var),4)
                R2_CI_plus = round(R2_value + np.sqrt(R2_std_dev + R2_var),4)
                MALAR_value = models[k]['MALAR'] / 1
                MALAR_var = variance_Summ[varpos]['MALAR']*2
                MALAR_std_dev = std_Dev_Summ[stdpos]['MALAR']/1
                MALAR_CI_minus = round(MALAR_value -np.sqrt (MALAR_std_dev+MALAR_var),4)
                MALAR_CI_plus = round(MALAR_value + np.sqrt(MALAR_std_dev+MALAR_var),4)
                MLAR_value = models[k]['MLAR'] / 1
                MLAR_std_dev = std_Dev_Summ[stdpos]['MLAR']/1
                MLAR_var = variance_Summ[varpos]['MLAR']*2
                MLAR_CI_minus = round(MLAR_value - np.sqrt(MLAR_std_dev+MLAR_var),4)
                MLAR_CI_plus = round(MLAR_value + np.sqrt(MLAR_std_dev+MLAR_var),4)

                print(fieldname, 'MAE:', round(mae_value,6), mae_CI_minus, mae_CI_plus)
                print(fieldname, 'PW20:', round(pw20_value,6),pw20_CI_minus, pw20_CI_plus)
                print(fieldname, 'R2:', round(R2_value,6), R2_CI_minus, R2_CI_plus)
                print(fieldname, 'MALAR:', round(MALAR_value,6), MALAR_CI_minus, MALAR_CI_plus)
                print(fieldname, 'MLAR:', round(MLAR_value,6), MLAR_CI_minus, MLAR_CI_plus)
