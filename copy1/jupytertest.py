...
def traineval(est: Estimator, xtrain, ytrain, xtest, ytest, squaring, df, randomseed):
    ...
    if est.identifier != "LR":  # tuning is not required for LR
        if est.identifier == "XGBR":
            ...
            def RF_Objective(trial):
                ...
                RF_model = RandomForestRegressor(max_depth=_max_depth, min_samples_split=_min_samples_split,
            ...
            # define parameter sp start = time.time()
            def DTR_objective(trial):
                ...
                DTR_model = DecisionTreeRegressor(min_samples_leaf=_min_samples_leaf, max_depth=_max_depth,
                                                  max_features=_max_features, max_leaf_nodes=_max_leaf_nodes)
            ...
            def LASSO_Objective(trial):
                ...
                _alpha = trial.suggest_categorical("alpha",
                                                   [0.006644814, 0.010214848, 0.000233117, 0.012706666, 0.009199882,
                                                    0.000377741, 0.00075485, 0.0000992077, 0.000528944])
            ...
            def RIDGE_Objective(trial):
                _alpha = trial.suggest_categorical("alpha", [2.236514, 0.060708, 0.204956, 0.133253, 0.512693, 1.269382,
            ...
            def ELNET_Objective(trial):
                ...
                _alpha = trial.suggest_categorical("alpha",
                                                   [0.000198094, 0.015176516, 0.0, 0.003264721, 0.00066628, 0.001536277,
                                                    0.0004508042, 0.002504531])
                _l1_ratio = trial.suggest_categorical("l1_ratio",
                                                      [0.023118993, 0.448966271, 0.664744323, 0.34692344, 0.550401256,
                                                       0.245397394, 0.4617206, 0.312367882, 0.236122903, 0.094718953,
                                                       0.631230102])
                _max_iter = trial.suggest_categorical("max_iter",
    ...
    if ml_learner != "LR":
        suffix = str(df).zfill(3)
        dfHyper = pd.read_csv(
            r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\OPTUNAHYPERPARAMETERS\model_" + ml_learner + suffix + ".csv", ";")
            ...
            for imp in range(impNumber):
                    ...
                    if root == 'C:\\Users\\Claire\\GIT_REPO_1\\CSCthesisPY\\WarImputations':
                        for file in files:
                            if runImp < maxImp and file.endswith('.csv') and (
                                    "train_" not in file and "test_" not in file and "SPLIT" not in file and
                                    "TRAIN" not in file and "TEST" not in file) and "ImpWarPATH" in file:
                ...
                if os.path.exists(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarImputations\TRAINSPLIT" + ".csv"):
                    if os.path.exists(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarImputations\TESTSPLIT" + ".csv"):
                        trainID = pd.read_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarImputations\TRAINSPLIT" +
                                              ".csv", ";")
                        testID = pd.read_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WarImputations\TESTSPLIT" +
                                             ".csv", ";")
                    ...
                    for i in dfResults.index:
                        ...
                        for i in range(len(metric_columns)):
                            for _, est in enumerate(estimates):
                                ...
                                metric_values = np.where(dfResults['Estimator'] == current_estimator,
                                                         dfResults[current_metric],
                                                         9999)
