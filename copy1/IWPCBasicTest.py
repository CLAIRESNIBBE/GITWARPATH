      ...
      if True:
        ...
        for i in range(len(metric_columns)):
          ...
          for k in range(len(models)):
            ...
            mae_std_dev = std_Dev_Summ[stdpos]['MAE'] / 1000
            ...
            pw20_std_dev = std_Dev_Summ[stdpos]['PW20'] / 1000
            pw20_var = variance_Summ[varpos]['MAE'] * 2
            pw20_CI_minus = round(pw20_value - 1.96 * (pw20_std_dev), 4)
            pw20_CI_plus = round(pw20_value + 1.96 * (pw20_std_dev), 4)
            R2_value = models[k]['R2'] / 1
            R2_var = variance_Summ[varpos]['R2'] * 2
            R2_std_dev = std_Dev_Summ[stdpos]['R2'] / 1000
            R2_CI_minus = round(R2_value - 1.96*(R2_std_dev) , 4)
            R2_CI_plus = round(R2_value + 1.96 * (R2_std_dev), 4)
            MALAR_value = models[k]['MALAR'] / 1
            MALAR_var = variance_Summ[varpos]['MALAR'] * 2
            MALAR_std_dev = std_Dev_Summ[stdpos]['MALAR'] / 1000
            MALAR_CI_minus = round(MALAR_value - 1.96 * (MALAR_std_dev) , 4)
            MALAR_CI_plus = round(MALAR_value + 1.96 * (MALAR_std_dev), 4)
            MLAR_value = models[k]['MLAR'] / 1
            MLAR_std_dev = std_Dev_Summ[stdpos]['MLAR'] / 1000
            MLAR_var = variance_Summ[varpos]['MLAR'] * 2
            MLAR_CI_minus = round(MLAR_value - 1.96 * (MLAR_std_dev), 4)
            MLAR_CI_plus = round(MLAR_value + 1.96 * (MLAR_std_dev), 4)

            print(fieldname, 'MAE:', round(mae_value, 6), "StdDev:", round(mae_std_dev,6), " CI: [",mae_CI_minus, mae_CI_plus,"]")
            print(fieldname, 'PW20:', round(pw20_value, 6),"StdDev:", round(pw20_std_dev,6)," CI: [", pw20_CI_minus, pw20_CI_plus,"]")
            print(fieldname, 'R2:', round(R2_value, 6), "StdDev:", round(R2_std_dev,6)," CI: [", R2_CI_minus, R2_CI_plus, "]")
            print(fieldname, 'MALAR:', round(MALAR_value, 6), "StdDev:", round(MALAR_std_dev,6), " CI: [", MALAR_CI_minus, MALAR_CI_plus,"]")
            print(fieldname, 'MLAR:', round(MLAR_value, 6), "StdDev", round(MLAR_std_dev,6), " CI:",MLAR_CI_minus, MLAR_CI_plus, "]")
