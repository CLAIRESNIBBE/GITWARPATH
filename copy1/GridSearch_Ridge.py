...
from sklearn.model_selection import GridSearchCV, cross_val_score, cross_validate, KFold, RepeatedKFold, RandomizedSearchCV
...
for imp in range(impNumber):
    ...
    for file in filesImp:
        ...
        if df == 1:
            ...
            # random search linear regression model
            ...
            space['alpha'] = np.linspace(0, 0.02, 11)
            space['fit_intercept'] = [True, False]
            space['normalize'] = [True, False]
            # define search
            search =  RandomizedSearchCV(model, space, n_iter=5000, scoring='neg_mean_absolute_error', n_jobs=-1, cv=cv,random_state=1)
            # execute search
            result = search.fit(x_train_poly_scaled, y_train)
            # summarize result
