...
def main():
    dfmain = pd.read_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\IWPC_Blacks.csv", ";")
    print(dfmain.info())
    ...
    for root, dirs, files in os.walk(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\Imputations"):
        for file in files:
          if file.endswith('.csv') :
            ...
            file_old = root + "\\"+file
            dotpos = file.find(".")
            fnew = file[0:7] + file[7:dotpos].zfill(3) + ".csv"
            filesImp1New.append(fnew)
            file_new = root + "\\"+fnew
    ...
    for root, dirs, files in os.walk(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\MICESTATSMODEL"):
      for file in files:
         if file.endswith('.csv') :
           ...
           file_old = root + "\\" + file
           dotpos = file.find(".")
           fnew = file[0:10] + file[10:dotpos].zfill(3) + ".csv"
           filesImp2New.append(fnew)
           file_new = root + "\\" + fnew
           os.rename(file_old, file_new)

    #filesImp2.sort()
    filesImp2New = []
    for file in filesImp2:
        dotpos = file.find(".")
        fold = file
        fnew = file[0:10] + file[10:dotpos].zfill(3) + ".csv"
        filesImp2New.append(fnew)
    ...
    for i in range(len(filesImp1)):
      ...
      df["Smoker"] = df.apply(lambda x: ConvertYesNo(x["Smoking_status"]), axis=1)
      df["Indicationflag"] = df.apply(lambda x: ConvertYesNo(x["Indication"]), axis=1)
      df.drop(["Inducer_status", "Amiodarone_status", "Smoking_status", "Indication"], axis=1, inplace=True)
      ...
      df["IndicationComp"] = dfcomp["Indicationflag"]
      df["InducerComp"]  = dfcomp["Inducer"]
      df["SmokerComp"] = dfcomp["Smoker"]
      ...
      df["IndicationMatch?"] = np.where(df['IndicationComp'] == df['Indicationflag'],0, df['Indicationflag']-df['IndicationComp'])
      df["SmokerMatch?"] = np.where (df['Smoker'] == df['SmokerComp'],0, df['Smoker']-df['SmokerComp'])
      df["AmiodaroneMatch?"] = np.where (df['Amiodarone'] == df['AmiodaroneComp'], 0, df['Amiodarone']-df['AmiodaroneComp'])
      df.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\IWPC_DFNEW_"  + str(i) + ".csv", ";")
