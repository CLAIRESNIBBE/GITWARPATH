...
def traineval(est: Estimator, grid, xtrain, ytrain, xtest, ytest, squaring):
    ...
    #scores = cross_val_score(est.estimator, xtrain, ytrain, scoring=mae_scorer, cv=cv, n_jobs=-1,verbose=1)
...
cv = cross_validate(estimator=pipeline, X=X_train, y=y_train, cv=5, scoring="neg_root_mean_squared_error",
