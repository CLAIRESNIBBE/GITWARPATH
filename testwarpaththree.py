...
def traineval(est: Estimator, grid, xtrain, ytrain, xtest, ytest, squaring):
    ...
    res=[]
    print(f'\n{est.identifier}...')
    #pipe = Pipeline(steps=[['scaler', MinMaxScaler], ['regressor', est.estimator]])
    #pipe.fit(xtrain, ytrain)
    mae_scorer = make_scorer(MAEScore)
    cv = KFold(n_splits=10, random_state=1, shuffle=True)
    gridinitial = grid[est.identifier]
    #cv_results = cross_validate(estimator=pipe, X=xtrain, y=ytrain, cv=5, scoring='neg_mean_squared_error',
    #                            return_train_score=True)
    search = GridSearchCV(est.estimator, gridinitial, scoring=mae_scorer,cv=cv)
    search.fit(xtrain, ytrain)
    # scores = cross_val_score(est.estimator, xtrain, ytrain, scorin# g=mae_scorer, cv=cv, n_jobs=-1,verbose=1)
...
def main():
    ...
    for file in filesImp:
        ...
        if True:
            ...
            scaler = MinMaxScaler()
            squaring = True
            train = train.drop([status_column, unnamed_column], axis=1)
            test = test.drop([status_column, unnamed_column], axis=1)
