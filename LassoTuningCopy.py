...
from sklearn.linear_model import RidgeCV, ElasticNetCV, LassoLarsCV, Lasso, ElasticNet, SGDRegressor, LassoCV
...
def main():
    ...
    if df == 1:
        ...
        # define grid
        grid = dict()
        grid['alg__alpha'] = arange(0, 1, 0.01)
        # define search
        search = GridSearchCV(estimator = pipeline_scaled, param_grid=grid, scoring='neg_mean_absolute_error', cv=cv, n_jobs=-1)
