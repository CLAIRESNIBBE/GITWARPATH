...
def main():
    ...
    dfmod = pd.read_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\All_comparisonModified.csv", ";")
    ...
    factor_WarPath = {'Intercept': 20.2832, 'Age': -0.0656, 'Weight': 0.2178, 'TargetINR': 7.3190, 'HIVPositive':8.7973, 'HIVUnknown':3.4054 }
    number_of_samples = 100
    number_of_imps = 100
    filelist = [f for f in os.listdir(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WARMICE") if f.endswith(".csv")]
    for f in filelist:
        os.remove(os.path.join(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WARMICE", f))
    results = []
    std_Dev = []
    ...
    for j in range(number_of_imps):
        ...
        x.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WARMICE\WARMICE_" + str(suffix) + ".csv", ";")

    filesImp = []
    for root, dirs, files in os.walk(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WARMICE"):
        for file in files:
    ...
    for file in filesImp:
      ...
      IWPCbase = factor_IWPC["Intercept"] + factor_IWPC["Age"] * dfmod["AgeDecades"] + factor_IWPC["Height"] * dfmod["Height_cm"] + \
      ...
      impResults.append({'Imp': df, 'model': 'Gage', 'predactual': dfKey, 'MAE': 0, 'PW20': 0, 'R2': 0,  'MLAR': 0,'MALAR': 0})
      dfmod["WarPATH_Pred"] = factor_WarPath['Intercept']+factor_WarPath['Age']*dfmod['AgeYears']+factor_WarPath['Weight']*dfmod['Weight_kg']+\
                              factor_WarPath['TargetINR']*dfmod['INR_Three']+\
                              factor_WarPath['HIVPositive']*dfmod['HIVPositive']+factor_WarPath['HIVUnknown']*dfmod['HIVUnknown']

      dfmod["WarPATH_Pred_Round"] = DoseRound(dfmod['WarPATH_Pred'])
      dfKey = dfmod[["WarPATH_Pred", "WarPATH_Pred_Round", "Dose_mg_week"]]
      impResults.append({'Imp': df, 'model': 'WarPATH', 'predactual': dfKey, 'MAE': 0, 'PW20': 0, 'R2': 0,  'MLAR': 0,'MALAR': 0})
      ...
      impResults.append({'Imp': df, 'model': 'Fixed', 'predactual': dfKey, 'MAE': 0, 'PW20': 0, 'R2': 0,  'MLAR': 0,'MALAR': 0})
      suffix = str(df).zfill(3)
      dfmod.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WARMICEPRED\WARImputed_" + suffix + ".csv", ";")
      ...
      if True:
        while boot < number_of_samples:
          ...
          dfMetricfactors=dfsample[["WarPATH_Pred","WarPATH_Pred_Round","IWPC_Pred","IWPC_Pred_Round","Fixed_Pred","Fixed_Pred_Round","Gage_Pred","Gage_Pred_Round","Dose_mg_week"]]
          suffix = str(df).zfill(3)
          dfsample.to_csv(r"C:\Users\Claire\GIT_REPO_1\CSCthesisPY\WAR_Imp" + suffix + "Samp" + str(boot) + ".csv", ";")
          boot = boot + 1
